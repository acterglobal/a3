...
// #[tokio::test]
// async fn news_external_link() -> Result<()> {
//     let _ = env_logger::try_init();
//     let (mut user, room_id) = random_user_with_random_space("news_link").await?;
//     let state_sync = user.start_sync();
//     state_sync.await_has_synced_history().await?;
...
//     let news_entry = space.latest_news_entries(1).await?.first().unwrap();

//     // generate the external and internal links

//     let internal_link = news_entry.internal_link();
//     let external_link = news_entry.external_link().await?;

//     let room_id = &news_entry.room_id().to_string()[1..];
//     let news_entry_id = &news_entry.event_id().to_string()[1..];

//     let path = format!("o/{room_id}/boost/{news_entry_id}");
