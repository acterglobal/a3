...
async fn test_room_join_rules() -> Result<()> {
    ...
    let join_rule = "knock";
    let rule_event_id = convo.set_join_rules(join_rule.to_owned()).await?;
        ...
        if let Some(diff) = stream.next().now_or_never().flatten() {
            match diff.action().as_str() {
                "PushBack" | "Set" => {
                    ...
                    if let Some(event_id) = match_msg(&value, "Changed", join_rule) {
    ...
    assert_eq!(
        found_event_id,
        Some(rule_event_id.to_string()),
        "Even after 30 seconds, room join rules not received",
    );
...
fn match_msg(msg: &TimelineItem, change: &str, new_val: &str) -> Option<String> {
    ...
    let Some(content) = event_item.room_join_rules_content() else {
        return None;
    };
