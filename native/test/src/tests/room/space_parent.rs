...
async fn test_space_parent() -> Result<()> {
    ...
    assert_eq!(
        ...
        "Even after 30 seconds, space parent event not received",
    );
...
fn match_msg(
    ...
    canonical_new_val: bool,
) -> Option<String> {
    if msg.is_virtual() {
        return None;
    }
    let event_item = msg.event_item().expect("room msg should have event item");
    let Some(content) = event_item.space_parent_content() else {
        return None;
    };
    ...
    let Some(chg) = content.canonical_change() else {
        return None;
    };
    if chg != canonical_change {
        return None;
    }
    if content.canonical_new_val() != canonical_new_val {
        return None;
    }
    event_item.event_id()
}
