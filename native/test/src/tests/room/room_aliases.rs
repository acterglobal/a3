...
use acter_core::{models::status::RoomAliasesContent, util::do_vecs_match};
use anyhow::Result;
...
async fn test_room_aliases() -> Result<()> {
    ...
    let (found_event_id, content) =
        found_result.expect("Even after 30 seconds, room aliases not received");
    assert_eq!(
        found_event_id,
        aliases_event_id.to_string(),
        "Incorrect event id",
    );
    ...
    assert_eq!(
        content.change(),
        Some("Set".to_owned()),
        "change of room aliases should be set"
    );
    assert!(
        do_vecs_match(&content.new_val(), &aliases),
        "new val of room aliases is invalid"
    );
    assert_eq!(
        content.old_val(),
        None,
        "old val of room aliases is invalid"
    );

    Ok(())
}

fn match_msg(msg: &TimelineItem) -> Option<(String, RoomAliasesContent)> {
    if msg.is_virtual() {
        return None;
    }
