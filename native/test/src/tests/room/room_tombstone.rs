...
async fn test_room_tombstone() -> Result<()> {
            ...
            match diff.action().as_str() {
                "PushBack" | "Set" => {
                    ...
                    if let Some(event_id) =
                        match_msg(&value, "Set", body, "Set", replacement_room_id.as_str())
                    {
    ...
    assert_eq!(
        found_event_id,
        Some(tombstone_event_id.to_string()),
        "Even after 30 seconds, room tombstone not received",
    );
...
fn match_msg(
    ...
    body_change: &str,
    body_new_val: &str,
    replacement_room_change: &str,
    replacement_room_new_val: &str,
) -> Option<String> {
    if msg.is_virtual() {
        return None;
    }
