...
async fn test_room_topic() -> Result<()> {
        ...
        if let Some(diff) = stream.next().now_or_never().flatten() {
            match diff.action().as_str() {
                "PushBack" | "Set" => {
                    ...
                    if let Some(event_id) = match_msg(&value, "Set", topic) {
    ...
    assert_eq!(
        found_event_id,
        Some(topic_event_id.to_string()),
        "Even after 30 seconds, room topic not received",
    );
...
fn match_msg(msg: &TimelineItem, change: &str, new_val: &str) -> Option<String> {
    ...
    let Some(content) = event_item.room_topic_content() else {
        return None;
    };
