...
async fn test_room_name() -> Result<()> {
    ...
    let name = "Babbling Room";
    let name_event_id = convo.set_name(name.to_owned()).await?;
    ...
    assert_eq!(
        found_event_id,
        Some(name_event_id.to_string()),
        "Even after 30 seconds, room name not received",
    );
...
fn match_msg(msg: &TimelineItem, change: &str, new_val: &str) -> Option<String> {
    ...
    let Some(content) = event_item.room_name_content() else {
        return None;
    };
