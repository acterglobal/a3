...
async fn room_msg_can_download_image() -> Result<()> {
    ...
    let draft = user
        ...
        .thumbnail_info(None, None, Some("image/png".to_owned()), Some(png_size))
            ...
            match diff.action().as_str() {
                "PushBack" | "Set" => {
                    let value = diff
                        .value()
                        .expect("diff pushback action should have valid value");
                    if let Some(msg_content) = match_media_msg(&value, mimetype, &mp3_name) {
                        let event_id = value
                            .event_item()
                            .and_then(|t| t.event_id())
                            .expect("media msg should be timeline event item");
