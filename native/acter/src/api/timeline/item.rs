...
impl TimelineEventItem {
    ...
    pub fn room_canonical_alias_content(&self) -> Option<RoomCanonicalAliasContent> {
        if let Some(TimelineEventContent::RoomCanonicalAlias(c)) = &self.content {
            Some(c.clone())
        } else {
            None
        }
    ...
    pub fn room_third_party_invite_content(&self) -> Option<RoomThirdPartyInviteContent> {
        if let Some(TimelineEventContent::RoomThirdPartyInvite(c)) = &self.content {
            Some(c.clone())
        } else {
            None
        }
