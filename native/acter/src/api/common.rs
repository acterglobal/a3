...
impl LocationDesc {
    pub fn new(body: String, geo_uri: String) -> Self {
        LocationDesc { body, geo_uri, thumbnail_source: None, thumbnail_info: None }
    }
    ...
    pub fn thumbnail_source(&self) -> Option<MediaSource> {
        self.thumbnail_source.clone().map(|inner| MediaSource { inner })
    }
    ...
    pub fn thumbnail_info(&self) -> Option<ThumbnailInfo> {
        self.thumbnail_info.clone().map(|inner| ThumbnailInfo { inner })
    }
}
