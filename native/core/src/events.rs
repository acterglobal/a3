...
use ruma_common::exports::{serde::de::Error as SerdeDeError, serde_json};
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        match &*ev_type {
            calendar::CalendarEventEventContent::TYPE => {
                let event = serde_json::from_str::<calendar::CalendarEventEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::CalendarEvent(event))
            }
            calendar::CalendarEventUpdateEventContent::TYPE => {
                let event = serde_json::from_str::<calendar::CalendarEventUpdateEvent>(json.get())
                    .map_err(D::Error::custom)?;
            ...
            pins::PinEventContent::TYPE => {
                let event =
                    serde_json::from_str::<pins::PinEvent>(json.get()).map_err(D::Error::custom)?;
                Ok(Self::Pin(event))
            }
            pins::PinUpdateEventContent::TYPE => {
                let event = serde_json::from_str::<pins::PinUpdateEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::PinUpdate(event))
            }

            news::NewsEntryEventContent::TYPE => {
                let event = serde_json::from_str::<news::NewsEntryEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::NewsEntry(event))
            }
            news::NewsEntryUpdateEventContent::TYPE => {
                let event = serde_json::from_str::<news::NewsEntryUpdateEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::NewsEntryUpdate(event))
            }

            tasks::TaskListEventContent::TYPE => {
                let event = serde_json::from_str::<tasks::TaskListEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::TaskList(event))
            }
            tasks::TaskListUpdateEventContent::TYPE => {
                let event = serde_json::from_str::<tasks::TaskListUpdateEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::TaskListUpdate(event))
            }

            tasks::TaskEventContent::TYPE => {
                let event = serde_json::from_str::<tasks::TaskEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::Task(event))
            }
            tasks::TaskUpdateEventContent::TYPE => {
                let event = serde_json::from_str::<tasks::TaskUpdateEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::TaskUpdate(event))
            }

            tasks::TaskSelfAssignEventContent::TYPE => {
                let event = serde_json::from_str::<tasks::TaskSelfAssignEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::TaskSelfAssign(event))
            }

            tasks::TaskSelfUnassignEventContent::TYPE => {
                let event = serde_json::from_str::<tasks::TaskSelfUnassignEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::TaskSelfUnassign(event))
            }

            comments::CommentEventContent::TYPE => {
                let event = serde_json::from_str::<comments::CommentEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::Comment(event))
            }
            comments::CommentUpdateEventContent::TYPE => {
                let event = serde_json::from_str::<comments::CommentUpdateEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::CommentUpdate(event))
            }

            attachments::AttachmentEventContent::TYPE => {
                let event = serde_json::from_str::<attachments::AttachmentEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::Attachment(event))
            }
            attachments::AttachmentUpdateEventContent::TYPE => {
                let event = serde_json::from_str::<attachments::AttachmentUpdateEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::AttachmentUpdate(event))
            }

            rsvp::RsvpEventContent::TYPE => {
                let event = serde_json::from_str::<rsvp::RsvpEvent>(json.get())
                    .map_err(D::Error::custom)?;
                Ok(Self::Rsvp(event))
            }
