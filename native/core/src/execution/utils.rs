...
mod tests {
    ...
    use super::super::traits::{Model, Store};
    use super::{default_model_execute, transition_tree};
    use crate::{config::TypeConfig, referencing::ExecuteReference};
    ...
    struct MockModel {
        id: MockObjectId,
        value: i32,
        parent_ids: Option<Vec<MockObjectId>>,
        transition_count: i32,
    }

    impl MockModel {
        fn new(id: &str, value: i32, parent_ids: Option<Vec<&str>>) -> Self {
            Self {
    ...
    impl Model<MockTypeConfig> for MockModel {
        ...
        async fn execute(
            ...
            Vec<ExecuteReference<MockTypeConfig>>,
            <Self::Store as Store<MockTypeConfig>>::Error,
        > {
    ...
    #[derive(Debug, Clone)]
    struct MockStore {
        models: Arc<RwLock<HashMap<MockObjectId, MockModel>>>,
    }
    ...
    impl MockStore {
        ...
        async fn insert(&self, model: MockModel) {
            let mut models = self.models.write().await;
            models.insert(model.object_id(), model);
        }

        async fn get_model(&self, id: &MockObjectId) -> Option<MockModel> {
