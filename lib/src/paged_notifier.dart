...
typedef LoadFunction<PageKeyType, ItemType> = Future<List<ItemType>?> Function(PageKeyType page, int limit);
typedef NextPageKeyBuilder<PageKeyType, ItemType> = PageKeyType? Function(List<ItemType>? lastItems, PageKeyType page, int limit);

class PagedNotifier<PageKeyType, ItemType> extends StateNotifier<PagedState<PageKeyType, ItemType>> with PagedNotifierMixin<PageKeyType, ItemType, PagedState<PageKeyType, ItemType>> {
  ...
  Future<List<ItemType>?> load(PageKeyType page, int limit) async {
    try {
      ...
      state = state.copyWith(
        records: [ ...(state.records ?? <ItemType>[]), ...(records ?? <ItemType>[]) ],
      ...
      if (mounted) {
        state = state.copyWith(
          error: errorBuilder != null ? errorBuilder!(e) : 'Si è verificato un\'errore. Per favore riprovare.'
        );
...
class NextPageKeyBuilderDefault<ItemType> {
  ...
  static NextPageKeyBuilder<int, dynamic> mysqlPagination = (List<dynamic>? lastItems, int page, int limit) {
