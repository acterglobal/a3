...
List<CommandShortcutEvent> codeBlockCommands({
  ...
  insertNewParagraphNextToCodeBlockCommand(localizations.codeBlockNewParagraph),
...
CommandShortcutEvent tabSpacesAtCurosrInCodeBlockCommand(String description) =>
    CommandShortcutEvent(
      ...
      getDescription: () => description,
      handler:
          (editorState) => _addTwoSpacesInCodeBlockCommandHandler(editorState),
    );
...
CommandShortcutEvent tabToInsertSpacesInCodeBlockCommand(String description) =>
    CommandShortcutEvent(
      ...
      handler:
          (editorState) =>
              _indentationInCodeBlockCommandHandler(editorState, true),
    );
...
///   - desktop
///   - web
CommandShortcutEvent tabToDeleteSpacesInCodeBlockCommand(String description) =>
    CommandShortcutEvent(
      ...
      handler:
          (editorState) =>
              _indentationInCodeBlockCommandHandler(editorState, false),
    );
...
///   - desktop
///   - web
CommandShortcutEvent selectAllInCodeBlockCommand(String description) =>
    CommandShortcutEvent(
...
///   - desktop
///   - web
CommandShortcutEvent pasteInCodeblock(String description) =>
    CommandShortcutEvent(
...
KeyEventResult _addTwoSpacesInCodeBlockCommandHandler(EditorState editorState) {
  final selection = editorState.selection;
  if (selection == null || !selection.isCollapsed) {
    return KeyEventResult.ignored;
  }
  ...
  final transaction =
      editorState.transaction..insertText(node, selection.end.offset, '  ');
...
KeyEventResult _indentationInCodeBlockCommandHandler(
  ...
  for (final line in lines) {
    if (!shouldIndent && line.startsWith(spaces) || shouldIndent) {
      ...
      shouldTransform =
          index + line.length >= selection.startIndex &&
...
CommandShortcutEventHandler _selectAllInCodeBlockCommandHandler = (
  ...
  editorState.service.selectionService.updateSelection(
    Selection.single(path: node.path, startOffset: 0, endOffset: delta.length),
  );
