...
class _RoomPageConsumerState extends ConsumerState<RoomPage> {
  ...
  Widget avatarBuilder(String userId) => AvatarBuilder(userId: userId);
          ...
          title: Column(
            ...
            children: [
              roomNotifier.asyncRoom.when(
                ...
                error: (err, st) => Text('Error loading room $err'),
                loading: () => const CircularProgressIndicator(),
              ),
            ...
            GestureDetector(
              onTap: () {
                context.pushNamed(
                  ...
                  pathParameters: {'roomId': widget.roomIdOrAlias},
                );
              },
              child: Padding(
                padding: const EdgeInsets.only(right: 10),
                child: SpaceParentBadge(
                  spaceId: widget.roomIdOrAlias,
                  badgeSize: 20,
                  child: roomNotifier.asyncRoom.when(
                    data: (room) {
                      final convoProfile =
                          ref.watch(chatProfileDataProvider(room));
                      return convoProfile.when(
                        data: (profile) {
                          return ActerAvatar(
                            uniqueId: widget.roomIdOrAlias,
                        ...
                        error: (err, stackTrace) {
                          ...
                          return ActerAvatar(
                            uniqueId: widget.roomIdOrAlias,
                            mode: DisplayMode.GroupChat,
                            displayName: widget.roomIdOrAlias,
                            size: 36,
                          );
                        },
                        loading: () => const CircularProgressIndicator(),
