...
class MembersSection extends ConsumerWidget {
  ...
  const MembersSection({super.key, required this.spaceId, this.limit = 3});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return membersLoader.when(
      ...
      error: (e, s) {
        ...
        return Center(child: Text(lang.loadingMembersFailed(e)));
      },
  ...
  Widget membersListUI(List<String> members, int count) {
    return ListView.builder(
      ...
      itemBuilder:
          (context, index) =>
              MemberListEntry(memberId: members[index], roomId: spaceId),
    );
  }
}
