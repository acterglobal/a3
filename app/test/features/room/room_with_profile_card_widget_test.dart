...
void main() {
  ...
  group('RoomWithAvatarInfoCard', () {
    ...
    testWidgets('shows parent badges when showParents is true',
      ...
      expect(find.byType(ActerAvatar), findsNWidgets(2)); // Avatar for the room and its parent
    });
    ...
    testWidgets('does not show parents badges when showParents is false',
      ...
      expect(find.byType(ActerAvatar), findsOneWidget); // Only one avatar (room)
    });
    ...
    testWidgets('shows visibility chip when showVisibilityMark is true',
        (WidgetTester tester) async {
      tester.binding.window.physicalSizeTestValue = Size(6000, 1200);
      await createWidgetUnderTest(
        tester: tester,
        roomId: 'room1',
        avatarInfo: avatarInfo,
      ...
      await tester.pumpAndSettle();
      expect(find.byKey(Key('visibility-chip')), findsOneWidget);
    });

    testWidgets('do not shows visibility chip when showVisibilityMark is false',
        (WidgetTester tester) async {
      await createWidgetUnderTest(
        tester: tester,
        roomId: 'room1',
        ...
        avatarSize: 40,
        contentPadding: EdgeInsets.zero,
        showVisibilityMark: false,
        showParents: false,
      );

      expect(find.byKey(Key('visibility-chip')), findsNothing);
    });

    testWidgets('shows bookmark indicator when showBookmarkedIndicator is true',
        (WidgetTester tester) async {
      await createWidgetUnderTest(
        tester: tester,
        roomId: 'room1',
    ...
    testWidgets('handles custom leading and trailing widgets',
      ...
      expect(find.byIcon(Icons.access_alarm), findsOneWidget); // Leading widget check
      expect(find.byIcon(Icons.accessibility_new), findsOneWidget); // Trailing widget check
    });
  });
}
